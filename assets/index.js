...
var firstJson;
var enableDate = new Array, disableDate = new Array;
$(function () {
    ...
    if ($(".product_json")[0]){
        ...
        var totalPerson = 0;
        var maxDate = new Date(moment(firstJson[firstJson.length - 1].start).format('YYYY-MM-DD'));

        var firstSlotStart = moment(firstJson[0].start, 'YYYY-MM-DD HH:mm:ss a');
        var firstSlotEnd = moment(firstJson[0].end, 'YYYY-MM-DD HH:mm:ss a');
        ...
        $(".pignose-calendar-button-schedule-pin.pignose-calendar-button-schedule-pin-offer").parent().parent().find("a").addClass("disabled-Color")
    }
...
var timesArray = [], datesAvailable, month, year, day, year1, month1, day1, calenderJson, getEndDate, getStartDate;
...
function dataMaker(date1) {
    ...
    for (var i=0; i<firstJson.length; i++){
        var startDate = new Date(firstJson[i].start).toISOString().slice(0, 10);
    ...
    for(var i=0; i<timesArray.length; i++){
        ...
        $("#available_time").append('<a data-booking="'+parseInt(timesArray[i].id)+'" data-start='+timesArray[i].start+' data-end='+timesArray[i].end+' data-person=' +parseInt(timesArray[i].Available)+ ' data-time="'+formatAMPM(new Date(getStartDate))+'-'+ formatAMPM(new Date(getEndDate))+'"  href="javascript:void(0)" class="dropdown-item time_dropdown_item">'+formatAMPM(new Date(getStartDate))+' - '+ formatAMPM(new Date(getEndDate))+'</a>');
    }
...
$("body").on("click", ".person_dropdown_list", function () {
    ...
    $(".product_price_calender").text('Rs. '+productPrice * parseInt($(this).attr('data-qty')));
...
$("body").on("click", ".minus_plus_master", function () {
    var quantityVal = parseInt($(this).parent(".sp-quantity").find(".quntity-input").val());
...
$("body").on("click", ".update_tour", function (e) {
    e.preventDefault();
    var metaFieldId;
    var startTiming = $(this).parent(".form_attributes").find("input[name='properties[start]']").val();
    var endTiming = $(this).parent(".form_attributes").find("input[name='properties[end]']").val();
    var id = $(this).parent(".form_attributes").find("input[name='properties[Id]']").val();
    var time = $(this).parent(".form_attributes").find("input[name='properties[Time]']").val();
    var orderId = $(this).attr("data-id");
    var lineItemId = $(this).parent(".form_attributes").find("input[name='id']").val();
    // return;
    var metaJson ='[{"id":"'+id+'", "line_item":  "'+lineItemId+'" , "startTiming": "'+startTiming+'", "endTiming": "'+endTiming+'", "time": "'+time+'}]';
    jQuery.ajax({
        type: 'POST',
        ...
        success: function (data) {
            ...
            jQuery.ajax({
                ...
                dataType: 'json',
                data: {'order_number': orderId, "metaFieldId": metaFieldId, "metaJson": metaJson },
                success: function (data) {
                    console.log(data);
